define([
    ...
    var
        dots;
    dots = [];
    dots.cookBatch = function cookBatch(number, worldPositionFromViewportPosition, randomViewportPosition, cameraPosition, metre) {
        var
            dot,
            i,
            length;
        for (i = 0, length = number; i < length; i += 1) {
            ...
            dot.colour = zones.getByPosition(vec2.length(dot.position) / metre).dotColour(dot.glimmer);
    ...
    dots.update = function update(viewportPosition, getBounds, cameraPosition, metre) {
        var
            dotWasRepositioned,
            i,
            length;
        for (i = 0, length = this.length; i < length; i += 1) {
            ...
            if (dotWasRepositioned && zones.getByPosition(vec2.length(this[i].drawingPosition) / metre).dotColour) {
                this[i].colour = zones.getByPosition(vec2.length(this[i].drawingPosition) / metre).dotColour(this[i].glimmer);
            }
        }
    }.bind(dots);
    return dots;
