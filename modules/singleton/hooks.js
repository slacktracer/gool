    ...
    hooks = {
        ...
        install: function (context) {
            ...
            for (i = 0, length = hooks.events.context.length; i < length; i += 1) {
                ...
                hooks[hooks.events.context[i]] = hooks.createRegisterFunction(hooks.events.context[i]);
                context[hooks.events.context[i]] = this.createHandlerFunction(hooks.events.context[i]);
            }
        },
        createHandlerFunction: function (eventName) {
            return function () {
                var
                    i,
                    length;
                for (i = 0, length = hooks.context[eventName].length; i < length; i += 1) {
